...
public class AggregateExpressionVisitor extends ExpressionVisitorAdapter {
	...
	@Override
	public void visit(Function function) { 	//Function means a aggregateOperation for a column?
		String f = function.getName();
		f = f.toLowerCase();				//Transfer to a common lowercase name, it is easy to judge
		switch (f) {
		case "avg":
		...
		//Get column name, if it is COUNT, column name doesn't matter just *
